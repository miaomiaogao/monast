...
class MonastHTTP(resource.Resource):
	...
	def getStatus(self, request):
		...
		for roomtypes, roomlist in server.status.meetmes.items():
			tmp[servername]['meetmes'][roomtypes] = []
			for roomname, room in roomlist.items():
				tmp[servername]['meetmes'][roomtypes].append(room.__dict__)
			tmp[servername]['meetmes'][roomtypes].sort(lambda x, y: cmp(x.get('meetme'), y.get('meetme')))

		# ## Confbridges
		# for confbridgeroom, confbridge in server.status.confbridges.items():
		# 	tmp[servername]['confbridges'].append(confbridge.__dict__)
		# tmp[servername]['confbridges'].sort(lambda x, y: cmp(x.get('confbridge'), y.get('confbridge')))
...
class Monast:
	...
	def _updateMeetme(self, servername, **kw):
		...
		try:
			...
			if user:
				...
				log.debug("Server %s :: Added user %s to Meetme %s %s", servername, user.get('usernum'), meetme.meetme,
						  _log)
			...
			if user:
				...
				if u:
					log.debug("Server %s :: Removed user %s from Meetme %s %s", servername, u.get('usernum'),
							  meetme.meetme, _log)
	...
	## Conference
	def _createConfbridge(self, servername, **kw):
		server = self.servers.get(servername)
		meetmeroom = kw.get('meetme')
		dynamic = kw.get("dynamic", False)
		forced = kw.get("forced", False)
		...
		if not meetme:
			...
			log.debug("Server %s :: Conference create: %s %s", servername, confbridge.meetme, _log)
			server.status.meetmes[meetmeroom] = meetme
			if dynamic:
				self.http._addUpdate(servername=servername, **meetme.__dict__.copy())
			if logging.DUMPOBJECTS:
			...
			log.warning("Server %s :: Conference already exists: %s", servername, meetme.meetme)

		return meetme


	def _updateConfbridge(self, servername, **kw):
		meetmeroom = kw.get("meetme")
		_log = kw.get('_log', '')
		try:
			meetme = self.servers.get(servername).status.meetmes.get(meetmeroom)
			...
			if user:
				meetme.users[user.get('usernum')] = user
				log.debug("Server %s :: Added user %s to Conference %s %s", servername, user.get('usernum'), meetme.meetme,
						  _log)
			...
			if user:
				u = meetme.users.get(user.get('usernum'))
				if u:
					log.debug("Server %s :: Removed user %s from Conference %s %s", servername, u.get('usernum'),
							  meetme.meetme, _log)
					del meetme.users[u.get('usernum')]
			...
			log.exception("Server %s :: Unhandled exception updating Conference: %s", servername, meetmeroom)


	def _removeConfbridge(self, servername, **kw):
		meetmeroom = kw.get("meetme")
		_log = kw.get('_log', '')
		try:
			server = self.servers.get(servername)
			meetme = server.status.meetmes.get(meetmeroom)
			if meetme:
				log.debug("Server %s :: Conference remove: %s %s", servername, meetme.meetme, _log)
				del server.status.meetmes[meetme.meetme]
				self.http._addUpdate(servername=servername, action='RemoveMeetme', meetme=meetme.meetme)
				if logging.DUMPOBJECTS:
					log.debug("Object Dump:%s", meetme)
			else:
				log.warning("Server %s :: Conference does not exists: %s", servername, meetmeroom)
		except:
			log.exception("Server %s :: Unhandled exception removing Conference: %s", servername, meetmeroom)
	...
	def __parseMonastConfig(self):
		...
		for server in servers:
			...
			self.servers[servername].confbridge_context = config.get(server, 'confbridge_context')
			self.servers[servername].confbridge_prefix = config.get(server, 'confbridge_prefix')
			...
			# self.servers[servername].status.confbridges  = {}
			...
			# self.servers[servername].displayConfbridges  = {}
		...
		for meetme, display in config.items('meetmes'):
			...
			if (self.displayMeetmesDefault and display == "hide") or (
					not self.displayMeetmesDefault and display == "show"):
		...
		# ## Conferences
		# self.displayConfbridgesDefault = config.get('confbridges', 'default') == 'show'
		# for confbridge, display in config.items('confbridges'):
		# 	if confbridge in ('default'):
		# 		continue
		#
		# 	servername, confbridge = confbridge.split('/', 1)
		# 	server = self.servers.get(servername)
		# 	if not server:
		# 		continue
		#
		# 	if (self.displayConfbridgesDefault and display == "hide") or (not self.displayConfbridgesDefault and display == "show"):
		# 		server.displayConfbridges[confbridge] = True
		#
		# 	if display == "force":
		# 		self._createConfbridge(servername, confbridge = confbridge, forced = True, _log = "By monast config")
	...
	def _requestAsteriskConfig(self, servername):
		...
		# Conferences
		log.debug("Server %s :: Requesting ConfbridgeRooms..." % servername)
		...
		def onGetConfbridgeConfig(result):
			log.debug("Server %s :: Processing conference.conf..." % servername)
			for k, v in result.items():
				if v.startswith("conf="):
					confbridgeroom = v.replace("conf=", "")
					if (self.displayMeetmesDefault and not server.displayConfbridges.has_key(confbridgeroom)) or (not self.displayMeetmesDefault and server.displayConfbridges.has_key(confbridgeroom)):
						self._createConfbridge(servername, confbridge = confbridgeroom)

		log.debug("Server %s :: Requesting conference conf..." % servername)
		server.pushTask(server.ami.getConfig, 'conference.conf') \
			.addCallbacks(onGetConfbridgeConfig, self._onAmiCommandFailure, errbackArgs = (servername, "Error Requesting Meetme.conf"))
	...
	def clientAction_Originate(self, session, action):
		...
		if type == "meetmeInviteUser":
			...
			originates.append(
				(channel, context, exten, priority, timeout, callerid, account, application, data, variable, async))
		...
		if type == "confbridgeInviteUser":
			application = "Confbridge"
			data        = "%s%sd" % (destination, [",", "|"][server.version == 1.4])
			originates.append((channel, context, exten, priority, timeout, callerid, account, application, data, variable, async))
			logs.append("Invite from %s to %s(%s)" % (channel, application, data))
		
		if type == "confbridgeInviteNumbers":
			dynamic     = not server.status.confbridges.has_key(destination)
			application = "Confbridge"
			data        = "%s%sd" % (destination, [",", "|"][server.version == 1.4])
			numbers     = source.replace('\r', '').split('\n')
			for number in [i.strip() for i in numbers if i.strip()]:
	...
	def clientAction_Transfer(self, session, action):
		...
		# if type == "confbridge":
		# 	extraChannel = action['extrachannel'][0]
		# 	exten        = "%s%s" % (server.confbridge_prefix, exten)
		# 	context      = server.confbridge_context
		#
		# 	if server.version == 1.8: ## Asterisk 1.8 requires some extra params
		# 		extraExten    = exten
		# 		extraContext  = context
	...
	def clientAction_MeetmeKick(self, session, action):
		...
		server.pushTask(server.ami.command, "meetme kick %s %s" % (meetme, usernum)) \
			.addErrback(self._onAmiCommandFailure, servername,
						"Error Executting Client Action Meetme Kick: %s -> %s..." % (meetme, usernum))

	def clientAction_ConfbridgeKick(self, session, action):  #to be verified...
		servername = action['server'][0]
		confbridge = action['confbridge'][0]
		usernum = action['usernum'][0]

		log.info("Server %s :: Executting Client Action Confbridge Kick: %s -> %s..." % (servername, confbridge, usernum))
		server = self.servers.get(servername)
		server.pushTask(server.ami.command, "confbridge kick %s %s" % (confbridge, usernum)) \
			.addErrback(self._onAmiCommandFailure, servername,
						"Error Executting Client Action Confbridge Kick: %s -> %s..." % (confbridge, usernum))
	...
	def handlerConfbridgeListRooms(self, ami, event):
		...
		server = self.servers.get(ami.servername)
		confbridgeroom = event.get('conference')
		user = '%s' %confbridgeroom
		if user:
			if (self.displayMeetmesDefault and not server.displayConfbridges.has_key(confbridgeroom)) or (
					not self.displayMeetmesDefault and server.displayConfbridges.has_key(confbridgeroom)):
				self._createConfbridge(ami.servername, confbridge=confbridgeroom)

				log.debug("Server %s :: Requesting Members of conference room %s ..." % (ami.servername, confbridgeroom))
				server.pushTask(ami.sendDeferred, {'action': 'ConfbridgeList', 'conference': confbridgeroom}) \
	...
	def handlerConfbridgeList(self, ami, event):
		...
		confbridge = event.get("conference")
		self._updateConfbridge(ami.servername, confbridge=confbridge,
							 addUser={
								# 'uniqueid': event.get('uniqueid'),
...
def RunMonast(MM):
	...
	_logHandler = logging.FileHandler(logfile, mode='w')  # using  mode='w' for test
